(function($) {
  ...
  function allCarsApp() {
    ...
    function removeCar(e) {
      ...
      removeCarFromServer(this.getAttribute('data-js-del'));
    }

    function removeCarFromServer(plateToRemove) {
      var del = new XMLHttpRequest();
      del.open('DELETE', 'http://localhost:3000/car/');
      del.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      del.send('plate='+plateToRemove+'');
      console.log('Apagando carro');
      del.onreadystatechange = function() {
        if( isRequestOk.call(this) ) {
          console.log('Carro removido', del.responseText);
        }
      };
    }
    ...
    function createCarInfoTableFromGet(element) {
        ...
        newTR.innerHTML += '<td><button type="submit" data-js="removeCar" data-js-del="'+carFieldsValues[3]+'">Remover</button></td>';
